...
def main(args):
    ...
    personalized_adapters2 = {uid: [{k: v.detach().cpu().clone() for k, v in layer.output.adapter.state_dict().items()} for layer in model.vilt.encoder.layer] for uid in range(n_users)}
    ...
    for ep in range(1, epochs+1):
        ...
        for uid in range(n_users):
            ...
            local_model = model
            # local_adapters =[{ k: v.clone() for k, v in layer_state.items() } for layer_state in personalized_adapters[tid][uid]]
        ...
        print('saving...')
        save_model_components(classifier_modules, personalized_adapters2, personalized_adapters)
...
def execute():
    ...
    parser.add_argument("--gaf",                   type=int,   default=5)
