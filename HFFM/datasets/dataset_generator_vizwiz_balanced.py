...
per_label_max = 800     # up to this many samples per label

# ─── LOAD & COUNT ────────────────────────────────────────────────────────────────
records = json.load(open(ann_file))
answers = [r["answers"][0]["answer"].strip().lower() for r in records]
answers = [a for a in answers if a and a != "unanswerable"]
...
for rec in records:
    ans = rec["answers"][0]['answer'].strip().lower()
